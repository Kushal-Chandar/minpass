...
DROGON_TEST(JsonProcessorTests_ParseRequestJson_ReturnValueTest1) {
  ...
  CHECK(password_data.email.get() == request["email"].asString());
  CHECK(password_data.username.get() == request["username"].asString());
  CHECK(password_data.password.get() == request["password"].asString());
  CHECK(password_data.master_password.get() ==
        request["master_password"].asString());
...
DROGON_TEST(JsonProcessorTests_ParseRequestJson_SQLi) {
  ...
  REQUIRE(request_data.has_value() == true);
  auto password_data = request_data.value();
  CHECK(password_data.email.get() == sql_injection["email"].asString());
  CHECK(password_data.username.get() == sql_injection["username"].asString());
  CHECK(password_data.password.get() == sql_injection["password"].asString());
  CHECK(password_data.master_password.get() ==
        sql_injection["master_password"].asString());
  CHECK(http_response->statusCode() == drogon::k202Accepted);
}
